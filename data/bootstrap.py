...
def _write_session_init_sql(data_dir: Path, question_ids: list[str]) -> None:
    init_path = data_dir / "session_init.sql"
    lines = [
    ...
    for qid in question_ids:
        lines.append(f"ATTACH 'data/{qid}.duckdb' AS {qid};")

    if question_ids:
        lines.append(f"USE {question_ids[0]}.main;")
    lines.append("")
...
def main() -> int:
    ...
    _write_session_init_sql(data_dir, all_ids)
    print("Done.")
    return 0
